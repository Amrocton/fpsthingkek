...
namespace Fragsurf.Movement {
    ...
    public class SurfCharacter : NetworkBehaviour, ISurfControllable {
        ...
        {
            _moveData = previous.moveData;
            _colliderObject.transform.rotation = Quaternion.identity;
            UpdateMoveData(playerInput.verticalAxis, playerInput.horizontalAxis, playerInput.wishJump);
        ...
        private void UpdateMoveData (float verticalAxis, float horizontalAxis, bool wishJump) {
            _moveData.verticalAxis = verticalAxis;
            _moveData.horizontalAxis = horizontalAxis;
        ...
        private PlayerInput GetPlayerInput()
        {
            ...
            playerInput.verticalAxis = Input.GetAxisRaw("Vertical");
            playerInput.horizontalAxis = Input.GetAxisRaw ("Horizontal");
            playerInput.wishJump = Input.GetButton ("Jump");
            return playerInput;
        }
        ...
        void SyncState()
        {
            if (isServer)
            {
                _moveData = state.moveData;
                transform.position = _moveData.playerTransform.position;
                transform.rotation = _moveData.playerTransform.rotation;
                return;
            }
            PlayerState stateToRender = isLocalPlayer ? predictedState : state;

            _moveData = stateToRender.moveData;
            transform.position = Vector3.Lerp(transform.position,
                stateToRender.moveData.playerTransform.position * Settings.PlayerLerpSpacing,
                Settings.PlayerLerpEasing);
            transform.rotation = Quaternion.Lerp(transform.rotation,
                stateToRender.moveData.playerTransform.rotation,
                Settings.PlayerLerpEasing);
        }
